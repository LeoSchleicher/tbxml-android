...
jlong Java_za_co_twyst_tbxml_TBXML_jniParse(JNIEnv* env,jobject object,jbyteArray xml) {
    ...
    document->length                 = N;
    document->xml                    = calloc(1,N+1); // initializes 1 block with N+1 bytes length
    ...
    __android_log_print(ANDROID_LOG_INFO,"TBXML","start parsing %d", N);
    
    document->xml = (*env)->GetByteArrayElements(env,xml,NULL);
    
    //for (int i = 0; i < N; ++i) {
    //__android_log_print(ANDROID_LOG_INFO,"TBXML","bytes[%d] = %x (%c)\n", i, ((const unsigned char *) document->xml)[i], ((const signed char *) document->xml)[i]);
    //}
 ...
 *
 */
jbyteArray Java_za_co_twyst_tbxml_TBXML_jniTextForElement(JNIEnv* env,jobject object,jlong document,jlong element) {
    TBXMLDocument *doc  = (TBXMLDocument *) (uintptr_t) document;
    TBXMLElement  *node = (TBXMLElement  *) (uintptr_t) element;
    ...
    if (doc) {
        if (node) {
            if (node->text) {
                // __android_log_print(ANDROID_LOG_INFO,"TBXML","Node [%s] Value %s",node->name,node->text);
                
                size_t length = strlen(node->text);
                
                jbyteArray array = (*env)->NewByteArray(env,length);
                (*env)->SetByteArrayRegion(env, array, 0, length, (const jbyte*)node->text);
                return array;
...
{ // ... initialise
    ...
    // __android_log_print(ANDROID_LOG_INFO,"TBXML","Source: %s",elementStart);
    ...
    while ((elementStart = strstr(elementStart,"<"))) {
        ...
        if (isCDATA == 0) {
            ...
            //__android_log_print(ANDROID_LOG_INFO,"TBXML","CDATALength: %ld textLength: %ld",CDATALength, textLength);
            
            //__android_log_print(ANDROID_LOG_INFO,"TBXML","before CDATA: %s",elementStart);
            //memcpy(elementStart,elementStart+9,CDATALength-9); // copies text from elementStart 9 symbols right
            //memcpy(CDATAEnd-9,CDATAEnd+3,textLength-CDATALength-3);
            //memset(elementStart+textLength-12,' ',12);
            
            // just fill CDATA with spaces
            memset(elementStart,' ',9);
            memset(CDATAEnd,' ',3);
            
            // __android_log_print(ANDROID_LOG_INFO,"TBXML","after CDATA: %s",elementStart);
            // __android_log_print(ANDROID_LOG_INFO,"TBXML","after CDATA: %s",CDATAEnd);
        ...
        while ((elementEnd = strpbrk(elementEnd, "<>"))) {// Returns a pointer to the first occurrence in elementEnd of any of the characters that are part of <>
        ...
        { elementStart = elementEnd+1;
            ...
            if (parentXMLElement) {
                if (parentXMLElement->text)
                {
                    ...
                    //__android_log_print(ANDROID_LOG_INFO,"TBXML","VALUE: %s", parentXMLElement->text);
                }
