...
jlong Java_za_co_twyst_tbxml_TBXML_jniParse(JNIEnv* env,jobject object,jbyteArray xml) {
    ...
    __android_log_print(ANDROID_LOG_INFO,"TBXML","Modified Version!");
                ...
                switch (mode) {
                    ...
                    case TBXML_ATTRIBUTE_VALUE_END:
                        if (*chr == '<' && strncmp(chr, "<![CDATA[", 9) == 0) {
                            ...
                            *chr = 0; // set end of string instead of closing quote
                            
                            while ((CDATAStart = strstr(value, "<![CDATA["))) {
                                __android_log_print(ANDROID_LOG_INFO,"TBXML","CDATA in ATTRIBUTE #1: %s",CDATAStart);
                                __android_log_print(ANDROID_LOG_INFO,"TBXML","Length of attribute: %i",strlen(CDATAStart));
                                //memcpy(CDATAStart, CDATAStart+9, strlen(CDATAStart)-8); // original method
                                //memset(CDATAStart,' ',9); // replace all CDATA with spaces
                                value = CDATAStart+9; // move attribute begin here
                                CDATAEnd = strstr(CDATAStart,"]]>");
                                //memcpy(CDATAEnd, CDATAEnd+3, strlen(CDATAEnd)-2); // original method
                                //memset(CDATAEnd,' ',3); // fill with spaces
                                *CDATAEnd = 0; // move end of string here
                                __android_log_print(ANDROID_LOG_INFO,"TBXML","CDATA in ATTRIBUTE #2: %s",value);
                            }
